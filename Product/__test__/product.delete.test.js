...
describe('DELETE /api/products/deleteproducts/:id (SELLER)', () => {
    let mongo;
    let sellerId1;
    let sellerId2;
    ...
    it('requires authentication (401) when no token provided', async () => {
        ...
        const res = await request(app)
            .delete(`/api/products/deleteproducts/${prod._id}`);
    ...
    it('returns 400 for invalid product id', async () => {
        ...
        const res = await request(app)
            .delete('/api/products/deleteproducts/not-a-valid-id')
            .set('Authorization', `Bearer ${token}`);
    ...
    it('returns 404 when product not found', async () => {
        ...
        const res = await request(app)
            .delete(`/api/products/deleteproducts/${missingId}`)
            .set('Authorization', `Bearer ${token}`);
    ...
    it("forbids deleting another seller's product (403)", async () => {
        ...
        const token = signToken(sellerId1.toHexString(), 'seller');
        const res = await request(app)
            .delete(`/api/products/deleteproducts/${otherProd._id}`)
            .set('Authorization', `Bearer ${token}`);
        expect(res.status).toBe(403);
    });
