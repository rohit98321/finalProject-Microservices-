...
describe('PATCH /api/products/updateproducts/:id (SELLER)', () => {
    let mongo;
    let sellerId1;
    let sellerId2;
    ...
    it('requires authentication (401) when no token provided', async () => {
        ...
        const res = await request(app)
            .patch(`/api/products/updateproducts/${prod._id}`)
            .send({ title: 'Nope' });
    ...
    it('returns 403 for invalid product id', async () => {
        ...
        const res = await request(app)
            .patch('/api/products/updateproducts/not-a-valid-id')
            .set('Authorization', `Bearer ${token}`)
            .send({ title: 'X' });
    ...
    it('returns 404 when product not found', async () => {
        ...
        const res = await request(app)
            .patch(`/api/products/updateproducts/${missingId}`)
            .set('Authorization', `Bearer ${token}`)
            .send({ title: 'New' });
    ...
    it("forbids updating another seller's product (403)", async () => {
        ...
        const token = signToken(sellerId1.toHexString(), 'seller');
        const res = await request(app)
            .patch(`/api/products/updateproducts/${otherProd._id}`)
            .set('Authorization', `Bearer ${token}`)
            .send({ title: 'Hack' });
        expect(res.status).toBe(403);
    });
