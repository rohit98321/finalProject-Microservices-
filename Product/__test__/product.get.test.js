...
describe('GET /api/products/getproducts', () => {
    ...
    it('supports text search via q parameter', async () => {
        ...
        const res = await request(app).get('/api/products/getproducts').query({ q: 'shirt' });
        expect(res.status).toBe(200);
    ...
    it('filters by minprice and maxprice', async () => {
        ...
        let res = await request(app).get('/api/products/getproducts').query({ minprice: '75' });
        expect(res.status).toBe(200);
        expect(res.body.data.length).toBe(2);

        res = await request(app).get('/api/products/getproducts').query({ maxprice: '120' });
        expect(res.status).toBe(200);
        expect(res.body.data.length).toBe(2);

        res = await request(app).get('/api/products/getproducts').query({ minprice: '60', maxprice: '120' });
        expect(res.status).toBe(200);
    ...
    it('supports pagination with skip and limit', async () => {
        ...
        let res = await request(app).get('/api/products/getproducts').query({ limit: '2' });
        expect(res.status).toBe(200);
        expect(res.body.data.length).toBe(2);

        res = await request(app).get('/api/products/getproducts').query({ skip: '2', limit: '2' });
        expect(res.status).toBe(200);
        expect(res.body.data.length).toBe(2);

        res = await request(app).get('/api/products/getproducts').query({ skip: '4', limit: '2' });
        expect(res.status).toBe(200);
        expect(res.body.data.length).toBe(1);
    });
});